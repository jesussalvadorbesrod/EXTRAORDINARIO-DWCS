...
// Importa el fichero con la función Jaxon
require_once 'infoVentas.php';
...
<head>
    ...
    <?php echo $jaxon->getCSS() . $jaxon->getJS() ?>
    <title>Tema 4</title>
</head>
<?php
...
<div class="container mt-3">
    ...
    <table class="table table-striped table-dark mt-3">
        ...
        <tr class="text-center">
            ...
            <!-- Agrega una columna donde irá la información de ventas -->
            <th scope="col"></th>
        ...
        while ($filas = $stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            // Agregado el evento para que se muestren las ventas al pulsar encima del producto
            $ventas = obtenerVentas($filas->id);
            $numeroProductos = $stmt->rowCount();
            echo "<td onclick='jaxon_mostrarVentas({$filas->id}, {$numeroProductos})'>";
            echo "<div >{$filas->nombre}, Precio: {$filas->pvp} (€)</div>";
            echo "</td>";

            /* Agrega la caja de texto o el texto indicando que no hay ventas y 
             * y la oculta. Sólo se mostrará si se pulsa el producto.
             */
            echo "<td class='text-center'>";
            if (count($ventas) >0) {
                echo "<textarea id='ventas-{$filas->id}' style='display: none;'>";
                foreach($ventas as $v) {
                    echo 'Empleado: ' . $v['empleado'] . ' Cliente: ' . $v['cliente'];
                }
                echo "</textarea>";
            } else {
                echo "<div id='ventas-{$filas->id}' style='display: none;'>SIN VENTAS</div>";
            }
            echo "</td>";
